...
const QuestAnswers = () => {
  ...
  return (
    <section className={styles.questanswer} id="faq">
      <div className="container">
        {
          !mediaIs1080 &&
          <>
            <h4 className={styles.title + " section-title"}>Questions and answers</h4>
          </>
        }
          ...
          <div className={styles.acc}>
            ...
            >
              <span>
                ...
                Just click on the button  <Link href={`${store.isAuth ? "/requests/create" : "/auth/login"}`}>«Create a request»</Link> and answer a few simple questions about the property you are looking for. It will only take a couple of minutes.
            ...
            >
              <span>
                ...
                To do so, just click on the button <Link href={`${store.isAuth ? "/objects/create" : "/auth/login"}`}>«Upload object»</Link> and answer a few questions about the real estate object you want to add. After that, the page of the object will be published in the<Link href={`${store.isAuth ? "/objects/my" : "/auth/login"}`}>“My Objects” </Link>  section.
                Important!<br />
